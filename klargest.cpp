...
int kth_largest(std::vector<int> values, int k) {
    ...
    if (k <= 0 || k > values.size()) {
        // Trả về giá trị Sentinel an toàn
    ...
    for (int value : values) {
        // 1. Nếu heap chưa đầy (kích thước < k), thêm phần tử vào.
        if (min_heap.size() < k) {
            min_heap.push(value);
        } else {
            // 2. Nếu heap đã đầy, so sánh giá trị hiện tại với phần tử nhỏ nhất trong heap (top).
    ...
    // 3. Sau khi duyệt hết, phần tử ở gốc của Min-Heap chính là phần tử lớn thứ k.
    return min_heap.top();
}
